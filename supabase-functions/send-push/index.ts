...
// Envia una notificacion Web Push a un trabajador (o a todos, con
// recipient_id: 'all'). Solo revisor/admin pueden invocarla (se valida el
// rol del que llama con su propio JWT). Usa service_role internamente para
// leer las suscripciones sin pasar por RLS, y deja registro en
// notifications_log.
...
Deno.serve(async (req) => {
  ...
  if (recipient_id === 'all') {
    ...
    for (const w of workers || []) {
      const { data: subs, error: subsErr } = await adminClient
        .from('push_subscriptions').select('*').eq('user_id', w.id);
      ...
      await adminClient.from('notifications_log').insert({
        recipient_id: w.id, type: type || 'broadcast', title, body: body || '', sent_by: userData.user.id
      });
    }
    return json(200, { sent: totalSent, workers: (workers || []).length, notified, errors });
  }
  ...
  const { sent, errors } = await sendToSubs(adminClient, (subs || []) as Sub[], title, body || '');
